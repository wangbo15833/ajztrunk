...
public class TsyjDAOImpl extends BasicObject
	implements TsyjDAO
{
	...
	public String createTsyj(Tsyj tsyj,HttpServletRequest request)
		throws BussinessProcessException
	{
		...
		try
		{
			...
			CachedRowSet cachedrowset = dbtrans.getResultBySelect("select colname from tabmap where tabname = 'tsyj' and isautoinc='0' order by CAST(colorder as numeric)");
			...
			StringBuffer stringbuffer = new StringBuffer("insert into tsyj(");
			...
			for ( int i = 0 ; i<al_col.size() ; i++){
				...
				stringbuffer.append("'");
				stringbuffer.append(tsyj.getProp(Tools.nvl(al_col.get(i).toString())));
				stringbuffer.append("'");
				bl=true;
			}
			stringbuffer.append(")");
			...
			hp_op.put("event","增加");
			hp_op.put("tabname","tsyj");
			hp_op.put("cent","insert tsyj");
	...
	public Query findTsyjList(Query query,HttpServletRequest request)
		throws BussinessProcessException
	{
		DBTrans dbtrans = new DBTrans();
		Object obj = null;
		...
		try
		{
			CachedRowSet cachedrowset = dbtrans.getResultBySelect("select colname from tabmap where tabname = 'tsyj' order by CAST(colorder as numeric)");
			if (cachedrowset==null||cachedrowset.size()==0){
				log.info("*** === there is no relate information in tabmap!  === ***");
				throw new BussinessProcessException("no information in tabmap");
			}
			...
			{
				StringBuffer sbr_sql = new StringBuffer("select * from tsyj where 1=1 ");
			...
			stringbuffer1.append("select top ").append(query.getRowsPerPage()).append(" * from tsyj where id not in ");
			stringbuffer1.append("(select top ").append(i).append(" id from tsyj where 1=1 ").append(sbr_condition.toString()).append(" order by id desc ) ");
	...
	public Tsyj findTsyjById(String id,HttpServletRequest request)
		throws BussinessProcessException
	{
		DBTransUtil dbtransutil = new DBTransUtil();
		Object obj = null;
		Tsyj tsyj = new Tsyj();
		StringBuffer stringbuffer = new StringBuffer("select * from tsyj where id =");
		...
		try
		{
			...
			{
				tsyj.setid(cachedrowset.getString("id"));
				for (int j = 1; j <= i; j++)
					tsyj.setProp(resultsetmetadata.getColumnName(j).toLowerCase(), Tools.nvl(cachedrowset.getString(j)));
			}
		}
		catch (Exception exception)
		{
	...
	public String modifyTsyj(Tsyj tsyj,HttpServletRequest request)
		throws BussinessProcessException
	{
		DBTrans dbtrans = new DBTrans();
		StringBuffer stringbuffer = new StringBuffer("update tsyj set ");
		ArrayList al_col = new ArrayList();
		try
		{
		...
		////以下根据tabmap开启外键！
			...
			String iuik = tabmapdao.setcascade(dbtrans,"tsyj");
		////以上根据tabmap开启外键！
			CachedRowSet cachedrowset = dbtrans.getResultBySelect("select colname from tabmap where tabname = 'tsyj' and isautoinc='0' order by CAST(colorder as numeric)");
			if (cachedrowset==null||cachedrowset.size()==0){
				log.info("*** === there is no relate information in tabmap!  === ***");
			...
			String s_col = null;
			for (int i=0 ; i<al_col.size() ; i++){
				s_col = Tools.nvl((String)tsyj.getProp(al_col.get(i).toString()));
				if (s_col != null && !s_col.equals("")){
					if(i!=0){
						stringbuffer.append(",");
					}
			...
			stringbuffer.append(tsyj.getid());
			log.info(stringbuffer.toString());
			dbtrans.addSql(stringbuffer.toString());
			boolean flag = dbtrans.executeSql();
			
		////以下根据tabmap关闭外键！
			tabmapdao.remcascade(dbtrans,"tsyj",iuik);
		////以上根据tabmap关闭外键！
			...
			hp_op.put("event","修改");
			hp_op.put("tabname","tsyj");
			hp_op.put("cent","update tsyj id ="+tsyj.getid());
			syslogdao.logact(hp_op,request,dbtrans);
	...
	public String deleteTsyj(String id,HttpServletRequest request)
		throws BussinessProcessException
	{
		DBTrans dbtrans = new DBTrans();
		int i = Integer.parseInt(id);
		StringBuffer stringbuffer = new StringBuffer("delete from tsyj where id = ");
		...
		try
		{
			...
			hp_op.put("event","删除");
			hp_op.put("tabname","tsyj");
			hp_op.put("cent","delete tsyj id ="+id);
			syslogdao.logact(hp_op,request,dbtrans);
	...
	{
		DBTrans dbtrans = new DBTrans();
		StringBuffer stringbuffer = new StringBuffer("select * from tabmap where 1=1 and tabname='tsyj' order by CAST(colorder as numeric)");
		try
		{				
		...
		throws BussinessProcessException
	{
		...
		try{
			...
			if(hp_user != null ){
				...
				DBTrans dbtrans = new DBTrans();
				String sql ="select * from pritab where tabname='tsyj' and role_name='"+role_name+"'";
