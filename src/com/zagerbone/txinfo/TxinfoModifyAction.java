...
public class TxinfoModifyAction extends BaseActionSupport
{

	public TxinfoModifyAction()
	{
	}
	...
	public EventResponse perform(Event event)
	{
		...
		System.out.println("***=== TxinfoModifyAction mode is "+ mode+" ===***");
		try
		{
			...
			System.out.println("***=== the QueryString in TxinfoFindListAction is "+queryString+" ===***");
			query.info();
			
			hashmap.put("id",id);
			hashmap.remove("mode");
			query.setConditionBody(hashmap);
			Query query1 = txinfodao.findTxinfoList(query,request);
			...
			Txinfo txinfo = (Txinfo)processEvent(event);
			String s = txinfodao.modifyTxinfo(txinfo,event.getRequest());
			eventresponse.setSuccessFlag(true);
		...
		throws BussinessProcessException
	{
		...
		HashMap hashmap = event.getBody();		
		txinfo.setid((String)hashmap.get("id"));
		hashmap.remove("id");
