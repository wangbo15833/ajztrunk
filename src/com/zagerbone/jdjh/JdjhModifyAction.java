...
public class JdjhModifyAction extends BaseActionSupport
{

	public JdjhModifyAction()
	{
	}
	...
	public EventResponse perform(Event event)
	{
		...
		System.out.println("***=== JdjhModifyAction mode is "+ mode+" ===***");
		try
		{
			...
			System.out.println("***=== the QueryString in JdjhFindListAction is "+queryString+" ===***");
			query.info();
			
			hashmap.put("id",id);
			hashmap.remove("mode");
			query.setConditionBody(hashmap);
			Query query1 = jdjhdao.findJdjhList(query,request);
			...
			Jdjh jdjh = (Jdjh)processEvent(event);
			String s = jdjhdao.modifyJdjh(jdjh,event.getRequest());
			eventresponse.setSuccessFlag(true);
		...
		throws BussinessProcessException
	{
		Jdjh jdjh = new Jdjh();
		HashMap hashmap = event.getBody();		
		jdjh.setid((String)hashmap.get("id"));
		hashmap.remove("id");
