...
//String tp_cargdzt= Tools.nvl(crs.getString("tp_cargdzt"));
%>
<head>
<title>生成报告</title>
...
function doCheck(){
	var ok = false; 
	var tp_carname = document.getElementById("tp_carname").value;
	if(tp_carname != ''){ 
		ok= window.confirm('您要将'+tp_carname+'归档,\n'+'确定吗?')	
	}	
	return ok;
}
</script>
...
<%
if(tp_cargdzt.equals("0")){
	%>
  <input type="button" name="button3" id="button3" value="车辆归档" onclick="doCheck(); doCargd();" />	
...
function doCargd(){
	var tp_testnum = document.getElementById('tp_testnum').value;
	var theUrl 		= "MainServlet_sys?fid=FCL00.00.00.042&tp_testnum="+tp_testnum;	
	var style			= "status:no;help:no;dialogWidth:65;dialogHeight:35;dialogTop:100;dialogLeft:250";
	//var theValue 	= window.showModalDialog(theUrl,null,style);
	winOpen(theUrl,700,400,50,50);
}
