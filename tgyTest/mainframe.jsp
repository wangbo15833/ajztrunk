...
String 	account = (String)hp_user.get("username");
   ...
   if(getObject(Id).className == "bg_image"){
      ...
      preClassName = Id;	//保持全局变量
			...
			if(navigator.userAgent.indexOf("MSIE")>0 ){
				...
				parent.frames['leftFrame'].MenuObj.getbytitle(sortname);
...
<div id="nav">
  ...
  <div style="float:right;padding-bottom:0px;text-align:bottom;" ><div style="float:left;margin-top:2px;"><img  src="../image2/icons/denied.png"></img></div><div style="float:left;"><font onclick="my_quit();"   style="cursor:pointer;" >退出系统</font>&nbsp;&nbsp;&nbsp;&nbsp;</div></div>
</div>
<script>
...
var constr = "../ajax/check.jsp?_="+Math.round(Math.random()*100000)+"&mode=newmsg&rec_account="+encodeURI("<%=account%>");
//alert(constr);
loadXMLDoc(constr,function()
  {
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    {
		...
		//alert(str);
		str=(str).replace(/[\r|\n]/gm,'');
		var str1 = "<A onclick=\"showman('../MainServlet_sys?fid=UII01.01.02.01&listMode=list&msgtype=通知&rec_account=<%=account%>')\"><img src='../image2/tranquiliti/file-note-paper.png' border='0' complete='complete'/> 通知（"+str.split(";")[0]+"）</A> <A onclick=\"showman('../MainServlet_sys?fid=UII01.01.02.01&listMode=list&msgtype=提醒&rec_account=<%=account%>')\"><img src='../image2/tranquiliti/Mail.png' border='0' complete='complete'/> 提醒（"+str.split(";")[1]+"）</A>"
...
<div style="float:left;width:100%" id="sub_info">&nbsp;登录人:<%=work_unit%>, <%=usertype%>, <%=username%>&nbsp;||&nbsp;&nbsp;&nbsp;&nbsp;<span id="show_text">建设工程安全管理服务系统信息系统!</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span id="msgnum" style="text-align:right;">未读消息数量……</span></div>
</body>
</html>
<script><%
if(curid==0){
