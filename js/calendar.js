...
//alert('in js write');
//document.write("<div style='position: absolute; z-index: 9999; top:200px;left:200px;background:#999;display:block;width:200px;height:200px;'>111111</br>111111111</br>111111111</br>111111111</br>111</div>");
...
$(document).click(function(ev){
    ...
    if(WebCalendar.eventSrc != e) hiddenCalendar();
...
function writeIframe()
{
    ...
    "</style><script src='./js/jquery.min.js'></script></head><body onselectstart='return false' style='margin: 0px' oncontextmenu='return false'><form name=meizz>";

    if (WebCalendar.drag){ strIframe += "<scr"+"ipt language=javascript>"+
    "var drag=false, cx=0, cy=0, o = parent.WebCalendar.calendar; $(document).bind('mousemove',function(event){"+
    "if(parent.WebCalendar.drag && drag){if(o.style.left=='')o.style.left=0; if(o.style.top=='')o.style.top=0;"+
    "o.style.left = parseInt(o.style.left) + window.event.clientX-cx;"+
    "o.style.top  = parseInt(o.style.top)  + window.event.clientY-cy;}});"+
    "$(document).bind('keydown',function(event){ switch(window.event.keyCode){  case 27 : parent.hiddenCalendar(); break;"+
...
//    for (var i=yearMin; i<=yearMax; i++) strIframe += "<option value='"+i+"'>"+i+"年</option>";
    ...
    "    <table id=tableDay height='120px' width='"+ cal_Width +"px' border=0 cellspacing=1 cellpadding=0>";
         for(var x=0; x<5; x++){
           strIframe += "<tr>";
           for(var y=0; y<7; y++)
             strIframe += "<td class=out id='meizzDay"+ (x*7+y) +"' name='meizzDay"+ (x*7+y) +"'></td>"; //"+ (x*7+y) +"
           strIframe += "</tr>";
         }
         strIframe += "<tr>";
    ...
    with(WebCalendar.iframe)
    {
        ...
        {
             WebCalendar.dayObj[i] = eval("meizzDay"+ i);//这是原来ie下的方法
            //WebCalendar.dayObj[i] = $("#meizzDay"+ i)[0];
           // alert("td[name='meizzDay"+ i+"']");
            //alert("$ is "+$("td[name='meizzDay"+ i+"']")[0]);
            //alert("tagname is "+$('#meizzDay'+ i)[0]);
			//WebCalendar.dayObj[i] = $("td[name='meizzDay"+ i+"']")[0];
			//alert($("td[name='meizzDay"+ i+"']")[0].innerHTML);
			//alert(WebCalendar.dayObj[i].innerHTML);
...
//alert(event);
	//try{
		//var ev1  =  ev  ||  window.event; // 事件 
		//var target    =  ev1.target  ||  ev1.srcElement; // 获得事件源
		var ev1;
		var e;
		//alert(ev);
		if(typeof window.event=="undefined"){
		...
		$(o).css("display","");//o.display = "";
		WebCalendar.iframe.document.body.focus();
		var cw = WebCalendar.calendar.clientWidth, ch = WebCalendar.calendar.clientHeight;
		var dw = document.body.clientWidth, dl = document.body.scrollLeft, dt = document.getElementById('man_zone').scrollTop;//lee *** snowcube
		if (document.body.clientHeight + dt - t - h >= ch)  $(o).css("top",(p=="image")? t + h -dt : t + h + 6 -dt) ;
		else $(o).css("top", (t - dt < ch) ? ((p=="image")? t + h : t + h + 6)-dt : t - ch - dt);
		if (dw + dl - l >= cw) $(o).css("left", l); else $(o).css("left", (dw >= cw) ? dw - cw + dl : dl);
		if  (!WebCalendar.timeShow) WebCalendar.dateReg = /^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2})$/;
		else WebCalendar.dateReg = /^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})$/;
		
		//alert($(o).css("top"));
	//}catch(e){alert("set pos \n"+e);}
    try{
...
//function String.prototype.trim(){return this.replace(new RegExp("/(^\s*)|(\s*$)/g"),"");}
...
$(".calendar").bind("focus",function(ev){calendar(ev);});
});