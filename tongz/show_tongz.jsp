...
import="java.io.*,com.jspsmart.upload.*"
%>
...
.file{color:green;}
.file:hover {color:lightblue;cursor:pointer;}
</style>
<script>
function download(filecode,filename){
	var url = "upload/download.jsp?"+"filecode="+filecode+"&"+"filename="+encodeURI(filename);
	alert(url);
	window.open(url);
}
</script>
...
<!-- <TABLE class=table style="BORDER-COLLAPSE: collapse" borderColor=#6595d6 cellSpacing=0 cellPadding=3 width="90%" align=center bgColor=#ffffff border=1>-->
	...
	while(ik<al_colname.size()){
			System.out.println("==="+(String)al_datasrctype.get(ik)+"===");
			if(ik%2==0){
		%>
	<tr><%}
		if(!((String)al_datasrctype.get(ik)).equals("textarea") && !((String)al_datasrctype.get(ik)).equals("enclosure")){
		%><td height="20" width="9%" align="right" class="text2"><%=(String)al_colnamec.get(ik)%>：</td><td width="41%" id="<%=(String)al_colname.get(ik)%>"><%=Tools.nvl(crs.getString(al_colname.get(ik).toString()))%></td>
		<%}else if(((String)al_datasrctype.get(ik)).equals("enclosure")){
		%><td height="20" width="9%" align="right" class="text2"><%=(String)al_colnamec.get(ik)%>：</td><td width="41%" id="<%=(String)al_colname.get(ik)%>"><%
		String files = crs.getString(al_colname.get(ik).toString());
		DBTrans dbtrans = new DBTrans();
		CachedRowSet crs_file = null;
		String sql ="";
		String fileName = "";
		String fileCode = "";
		if(files.equals("")){%>无<%}else{
			%><div class = 'fileList'><%
			...
			for (int i=0 ; i<f.length ; i++){
				...
				if(crs_file.size()>0){
					...
					%><div class = 'file'><span  onclick="javascript:download('<%=fileCode%>','<%=fileName%>')"><%=fileName%>&nbsp;&nbsp;&nbsp;下载</span></div><%
				}
			}
			%></div><%
		}
		...
		%><td height="20"  width="9%" align="right" class="text2"><%=(String)al_colnamec.get(ik)%>：</td><td  width="91%" colspan="3" ><%=Tools.nvl(crs.getString(al_colname.get(ik).toString())).replaceAll("%40%40","</br>")%></td>
		<%}
		%><%
		ik++;
		if(ik%2==0){
...
/*
  function checkreadlist(){
    ...
    document.getElementById("rec_name").innerHTML=cont;
	}
	checkreadlist();
	*/
  function doReturn()
