...
/* 	if(operationMode== 'input' ){ 
	...
	if(operationMode== 'delete' ){ 
		ok= window.confirm('您将要删除人员信息，与其绑定用户将会一同被删除，该用户将不能登录系统！\n'+'您确定删除吗?')	
	}	
	if(ok==false){
		return false;
	}

	submitframe.location = "MainServlet_sys?fid=UII00.00.03.02&id="+id_value;
	...
	<tr>
		<td bgcolor='#dedfd7'>
			...
			<form name="frmList" target="submitframe" ></form>
				...
				%>
				<tr align="center"> 
					<td height="15" colspan="<%=Integer.parseInt(mypri)>1?al_colname.size()+1:al_colname.size()%>"><p align="center" class="text2">未查询到人员信息</p></td>
				</tr>
				<%
				}
				else
				...
				<tr bgcolor='FFFFFF' class='font_px12_menu' id="tr_<%=Tools.nvl(crs.getString("id"))%>" onMouseOver="javascript:this.style.background= '#ECF2FC';" onMouseOut="javascript:this.style.background= '#FFFFFF';">
					...
					<td height="2" align="center" class="text3" nowrap="nowrap" id="tr_<%=i_row%>_edit2">
						<a href="#" onclick="show('MainServlet_sys?fid=UII00.00.03.05&id=<%=crs.getString("id")%>')"><font color="#053080">详细</font></a>
						<%if(Integer.parseInt(mypri)>2){%><a href="#" onclick="modify('MainServlet_sys?fid=UII00.00.03.04&id=<%=crs.getString("id")%>&mode=before')"><font color="#053080">修改</font></a>
...
$('.font_px12_menu').contextMenu('myMenu1', {
  bindings: {
	...
	'edit': function(t) {
	  //alert('Trigger was '+t.id+'\nAction was edit');
	  modify('MainServlet_sys?fid=UII00.00.03.04&mode=before&id='+ t.id.split("_")[1])
	},
	'save': function(t) {
	  alert('Trigger was '+t.id+'\nAction was Save');
	},
