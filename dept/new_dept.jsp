...
function myFunction(colname,val)
{
if (colname=="admin_account"){
	if(isRegisterUserName(val)){
		
	}else{
		alert("账号应为3-20位的 字母 数字 . _ 组合并且以字母开头，请检查格式");
		err3="账号格式不正确";
		document.getElementById("c_admin_account").innerHTML="3-20位的字母数字组合，例:zhangsan123";
		return false;
	}
}
if(val=='')
...
loadXMLDoc("./ajax/check.jsp?_="+Math.round(Math.random()*100000)+"&mode=checkname&col="+colname+"&val="+val,function()
  {
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    {
		var str = (xmlhttp.responseText);
		str=(str).replace(/[\r|\n]/gm,'');
		if(str.split(":")[0]=="dept_name"){
			if(str.split(":")[1]=="0"){
				document.getElementById("c_dept_name").innerHTML="";
				err1="";
			}else{
				document.getElementById("c_dept_name").innerHTML="企业名称重复，请用其它名称";
				err1="企业名重复";
			}
		}
		if(str.split(":")[0]=="admin_name"){
			if(str.split(":")[1]=="0"){
				document.getElementById("c_admin_name").innerHTML="";
				err2="";
			}else{
				document.getElementById("c_admin_name").innerHTML="用户名重复，请用其它名称";
				err2="用户名重复";
			}
		}
		if(str.split(":")[0]=="admin_account"){
			if(str.split(":")[1]=="0"){
				document.getElementById("c_admin_account").innerHTML="";
				err3="";
			}else{
				document.getElementById("c_admin_account").innerHTML="账号名重复，请用其它名称";
				err3="管理员账号重复";
			}
		}
		/* if (str == "repeat")
...
<form name="formHead" action="./MainServlet_sys" target="sframe"  method="post" onKeyDown="if(event.keyCode==13)event.keyCode=9" >
    <input type="hidden" name="fid" value="UII00.00.02.03">
	...
	<tr><%}%>		
		...
		%><td height="20" width="9%" align="right" class="text2"><%=(String)al_colnamec.get(ik)%>：</td><td width="41%"><input type="text" id="<%=al_colname.get(ik).toString()%>" name="<%=al_colname.get(ik).toString()%>" onblur="myFunction('<%=al_colname.get(ik).toString()%>',this.value)" size="25" value=""/><span style="color:red;" id="c_<%=al_colname.get(ik).toString()%>"></span></td> 
...
<script language="javascript">
	function  checknow()
	{
		if(<%=flag1%>){
		alert("增加成功！\n单位管理员的初始密码为123456");
		window.top.indexFrame.frames['manFrame'].location = 'MainServlet_sys?fid=UII00.00.02.01<%=Integer.parseInt(mypri)==4?"":filter_str%>';
		}
	}
	checknow();
</script>
