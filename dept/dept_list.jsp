...
/* 	if(operationMode== 'input' ){ 
	...
	if(operationMode== 'delete' ){ 
		ok= window.confirm('您将从系统删除该单位信息，该单位的人员信息，用户信息也将被删除\n\n'+'您确定吗?')	
	}	
	if(ok==false){
		return false;
	}

	submitframe.location = "MainServlet_sys?fid=UII00.00.02.02&id="+id_value;
	...
	<tr>
		<td bgcolor='#dedfd7'>
			...
			<form name="frmList" target="submitframe" ></form>
				...
				%>
				<tr align="center"> 
					<td height="15" colspan="<%=Integer.parseInt(mypri)>1?al_colname.size()+1:al_colname.size()%>"><p align="center" class="text2">未查询到单位信息</p></td>
				</tr>
				<%
				}
				else
				...
				<tr bgcolor='FFFFFF' class='font_px12_menu' id="tr_<%=crs.getString("id")%>" onMouseOver="javascript:this.style.background= '#ECF2FC';" onMouseOut="javascript:this.style.background= '#FFFFFF';">
					...
					<td height="2" align="center" class="text3" nowrap="nowrap" id="tr_<%=i_row%>_edit2">
						<a href="#" onclick="show('MainServlet_sys?fid=UII00.00.02.05&id=<%=crs.getString("id")%>')"><font color="#053080">详细</font></a>
						<%if(Integer.parseInt(mypri)>2){%><a href="#" onclick="modify('MainServlet_sys?fid=UII00.00.02.04&id=<%=crs.getString("id")%>&mode=before')"><font color="#053080">修改</font></a>
						<%}if(Integer.parseInt(mypri)>3){%><a href="#" onclick="confirmButton('delete', 0, <%=crs.getString("id")%>);"><font color="#ff0000" class="tdrecord" style="font-size:12px" >删除</font></a>
						<%}%>
					</td>
					<%
					}
...
</body><div class="contextMenu" id="myMenu1" style="">
  <ul><%
	//通过单位信息进行身份判断 
	if(Integer.parseInt(mypri)>1){
	%>
...
$('.font_px12_menu').contextMenu('myMenu1', {
  bindings: {
	'see': function(t) {
	  ...
	  show('MainServlet_sys?fid=UII00.00.02.05&id='+ t.id.split("_")[1])
	},
	'edit': function(t) {
	  //alert('Trigger was '+t.id+'\nAction was edit');
	  modify('MainServlet_sys?fid=UII00.00.02.04&mode=before&id='+ t.id.split("_")[1])
	},
	'save': function(t) {
	  alert('Trigger was '+t.id+'\nAction was Save');
	},
