...
<script language="JScript" type="text/javascript"  src="./js/idCard.js"> </script>

</head>


<%
String titleName="个人资料修改";
...
try{
  ...
  {  
	...
	String hideSeg="isblind;imsi;emp_bz";
	
	while(crs_cols.next()){
		if(Tools.nvl(crs_cols.getString("deta_disp")).equals("1")){	
			if(((String)crs_cols.getString("colname")).equals("emp_role") && Integer.parseInt(mypri)<4){continue;}
				
			if(hideSeg.indexOf(crs_cols.getString("colname"))>-1){
				continue;
			}
			al_colname.add(crs_cols.getString("colname"));
			al_colnamec.add(crs_cols.getString("colnamec"));
			al_datasrc.add(crs_cols.getString("datasrc"));
			al_datasrctype.add(crs_cols.getString("datasrctype"));
		}
	}
 %>
...
<!-- <TABLE class=table style="BORDER-COLLAPSE: collapse" borderColor=#6595d6 cellSpacing=0 cellPadding=3 width="90%" align=center bgColor=#ffffff border=1>-->
	...
	<input type="hidden" name="fid" id="fid" value="UII00.00.03.08"><input type="hidden" name="id" id="id" value="<%=crs.getString("id")%>">
		<%
	int ik = 0;
...
<script language=javascript>
  ...
  if(document.getElementById("cont")!=null){
	...
	document.getElementById("cont").value=s;}
    formHead.target = "submitframe";
	document.getElementById("fid").value="UII00.00.03.03";
	formHead.submit();
	//alert("已提交");
  }
  function doClose()
  ...
  function checkfilter()
  {
	<%if(Integer.parseInt(mypri)==3){%>
		...
		%>
	$("#emp_role,#emp_dept,#emp_lb").attr("disabled",true);
  }
  checknow();
  checkfilter();
</script>
...
$("#idcard").bind("blur",function(){
	...
	if (!checkFlag.IsValid()) {
		alert("输入的身份证号无效,请输入仔细核对！");
		$("#idcard").css("background","#e88");
	}else{
		$("#idcard").css("background","#fff");
	} 
});
});
</script>
...
<script language="javascript">
	function  checknow()
	{
		if(<%=flag1%>){
		alert("个人资料修改成功！");
		window.top.indexFrame.frames['manFrame'].location = 'MainServlet_sys?fid=UII00.00.03.06';
		}
	}
	checknow();
</script>
