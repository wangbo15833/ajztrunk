...
function confirmButton(operationMode, tr_row,id_value){
	...
	submitframe.location = "MainServlet_sys?fid=UII00.00.01.02&id="+id_value;
	return ok;
}
</script> 
</head>
...
<table width='100%' border='0' align='center' cellpadding='1' cellspacing='1' bordercolor='#bdd7d6' bgcolor='#bdd7d6'  rules='none' style="font-size:12px;">
	<tr id='sub_title'><td align="center"><font size="+1">单位类型列表</font></td></tr>
</table>
			...
			<form name="frmList" target="submitframe"></form>
				...
				<%
					for(int jk=0; jk<al_colname.size(); jk++){
						...
						%><td width = "40px" align="left" class="center_title_1" nowrap="nowrap">&nbsp;&nbsp;<%=(String)al_colnamec.get(jk)%></TD>
						<%
						}else{
						%><td align="left" class="center_title_1" nowrap="nowrap">&nbsp;&nbsp;<%=(String)al_colnamec.get(jk)%></TD>
						<%}
					}
				%><td align="center" width="60px" class="center_title_1" nowrap="nowrap"><%if(usertype.equals("系统管理员")){%>操&nbsp;&nbsp;作<%}else{%>无操作权限<%}%></td>
				</tr>
				<%
				...
				%>
				<tr align="center"> 
					<td height="15" colspan="<%=al_colname.size()+1%>"><p align="center" class="text2">未查询到单位类型</td>
				</tr>
				<%
				}
				else
				...
				<tr bgcolor='FFFFFF' class='font_px12' id="tr_<%=i_row%>" onMouseOver="javascript:this.style.background= '#ECF2FC';" onMouseOut="javascript:this.style.background= '#FFFFFF';">
					...
					<td height="2" align="center" class="text3" nowrap="nowrap" id="tr_<%=i_row%>_edit2">
						<a href="#" onclick="modify('MainServlet_sys?fid=UII00.00.01.04&id=<%=crs.getString("id")%>&mode=before')"><font color="#053080">修改</font></a>
