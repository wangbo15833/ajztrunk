...
public class QzbaModifyAction extends BaseActionSupport
{

	public QzbaModifyAction()
	{
	}
	...
	public EventResponse perform(Event event)
	{
		...
		System.out.println("***=== QzbaModifyAction mode is "+ mode+" ===***");
		try
		{
			...
			System.out.println("***=== the QueryString in QzbaFindListAction is "+queryString+" ===***");
			query.info();
			
			hashmap.put("id",id);
			hashmap.remove("mode");
			query.setConditionBody(hashmap);
			Query query1 = qzbadao.findQzbaList(query,request);
			...
			Qzba qzba = (Qzba)processEvent(event);
			String s = qzbadao.modifyQzba(qzba,event.getRequest());
			eventresponse.setSuccessFlag(true);
		...
		throws BussinessProcessException
	{
		Qzba qzba = new Qzba();
		HashMap hashmap = event.getBody();		
		qzba.setid((String)hashmap.get("id"));
		hashmap.remove("id");
