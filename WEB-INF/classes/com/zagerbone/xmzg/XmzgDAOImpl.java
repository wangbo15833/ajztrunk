...
public class XmzgDAOImpl extends BasicObject
	implements XmzgDAO
{
	...
	public String createXmzg(Xmzg xmzg,HttpServletRequest request)
		throws BussinessProcessException
	{
		...
		try
		{
			...
			CachedRowSet cachedrowset = dbtrans.getResultBySelect("select colname from tabmap where tabname = 'xmzg' and isautoinc='0' order by CAST(colorder as numeric)");
			...
			StringBuffer stringbuffer = new StringBuffer("insert into xmzg(");
			...
			//通过 xmname 获得 xmcode 就是subinfo 中的sub_code
				log.info("adminstrator insert xmzg xmname is :"+(String)xmzg.getProp("xmname"));
				CachedRowSet crs = dbtrans.getResultBySelect("select * from subinfo where gcname = '"+(String)xmzg.getProp("xmname")+"' ");
				crs.next();
				xmzg.setProp("xmcode",crs.getString("sub_code"));
			...
			for ( int i = 0 ; i<al_col.size() ; i++){
				...
				stringbuffer.append("'");
				stringbuffer.append(xmzg.getProp(Tools.nvl(al_col.get(i).toString())));
				stringbuffer.append("'");
				bl=true;
			}
			stringbuffer.append(")");
			...
			hp_op.put("event","增加");
			hp_op.put("tabname","xmzg");
			hp_op.put("cent","insert xmzg");
	...
	public Query findXmzgList(Query query,HttpServletRequest request)
		throws BussinessProcessException
	{
		DBTrans dbtrans = new DBTrans();
		Object obj = null;
		...
		try
		{
			CachedRowSet cachedrowset = dbtrans.getResultBySelect("select colname from tabmap where tabname = 'xmzg' order by CAST(colorder as numeric)");
			if (cachedrowset==null||cachedrowset.size()==0){
				log.info("*** === there is no relate information in tabmap!  === ***");
				throw new BussinessProcessException("no information in tabmap");
			}
			...
			{
				StringBuffer sbr_sql = new StringBuffer("select * from xmzg where 1=1 ");
			...
			stringbuffer1.append("select top ").append(query.getRowsPerPage()).append(" * from xmzg where id not in ");
			stringbuffer1.append("(select top ").append(i).append(" id from xmzg where 1=1 ").append(sbr_condition.toString()).append(" order by id desc ) ");
	...
	public Xmzg findXmzgById(String id,HttpServletRequest request)
		throws BussinessProcessException
	{
		DBTransUtil dbtransutil = new DBTransUtil();
		Object obj = null;
		Xmzg xmzg = new Xmzg();
		StringBuffer stringbuffer = new StringBuffer("select * from xmzg where id =");
		...
		try
		{
			...
			{
				xmzg.setid(cachedrowset.getString("id"));
				for (int j = 1; j <= i; j++)
					xmzg.setProp(resultsetmetadata.getColumnName(j).toLowerCase(), Tools.nvl(cachedrowset.getString(j)));
			}
		}
		catch (Exception exception)
		{
	...
	public String modifyXmzg(Xmzg xmzg,HttpServletRequest request)
		throws BussinessProcessException
	{
		DBTrans dbtrans = new DBTrans();
		StringBuffer stringbuffer = new StringBuffer("update xmzg set ");
		ArrayList al_col = new ArrayList();
		try
		{
		...
		////以下根据tabmap开启外键！
			...
			String iuik = tabmapdao.setcascade(dbtrans,"xmzg");
		////以上根据tabmap开启外键！
			CachedRowSet cachedrowset = dbtrans.getResultBySelect("select colname from tabmap where tabname = 'xmzg' and isautoinc='0' order by CAST(colorder as numeric)");
			if (cachedrowset==null||cachedrowset.size()==0){
				log.info("*** === there is no relate information in tabmap!  === ***");
			...
			String s_col = null;
			for (int i=0 ; i<al_col.size() ; i++){
				s_col = Tools.nvl((String)xmzg.getProp(al_col.get(i).toString()));
				if (s_col != null && !s_col.equals("")){
					if(i!=0){
						stringbuffer.append(",");
					}
			...
			stringbuffer.append(xmzg.getid());
			log.info(stringbuffer.toString());
			dbtrans.addSql(stringbuffer.toString());
			boolean flag = dbtrans.executeSql();
			
		////以下根据tabmap关闭外键！
			tabmapdao.remcascade(dbtrans,"xmzg",iuik);
		////以上根据tabmap关闭外键！
			...
			hp_op.put("event","修改");
			hp_op.put("tabname","xmzg");
			hp_op.put("cent","update xmzg id ="+xmzg.getid());
			syslogdao.logact(hp_op,request,dbtrans);
	...
	public String deleteXmzg(String id,HttpServletRequest request)
		throws BussinessProcessException
	{
		DBTrans dbtrans = new DBTrans();
		int i = Integer.parseInt(id);
		StringBuffer stringbuffer = new StringBuffer("delete from xmzg where id = ");
		...
		try
		{
			...
			hp_op.put("event","删除");
			hp_op.put("tabname","xmzg");
			hp_op.put("cent","delete xmzg id ="+id);
			syslogdao.logact(hp_op,request,dbtrans);
	...
	{
		DBTrans dbtrans = new DBTrans();
		StringBuffer stringbuffer = new StringBuffer("select * from tabmap where 1=1 and tabname='xmzg' order by CAST(colorder as numeric)");
		try
		{				
		...
		throws BussinessProcessException
	{
		...
		try{
			...
			if(hp_user != null ){
				...
				DBTrans dbtrans = new DBTrans();
				String sql ="select * from pritab where tabname='xmzg' and role_name='"+role_name+"'";
