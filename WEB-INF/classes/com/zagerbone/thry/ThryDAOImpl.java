...
public class ThryDAOImpl extends BasicObject
	implements ThryDAO
{
	...
	public String createThry(Thry thry,HttpServletRequest request)
		throws BussinessProcessException
	{
		...
		try
		{
			...
			CachedRowSet cachedrowset = dbtrans.getResultBySelect("select colname from tabmap where tabname = 'thry' and isautoinc='0' order by CAST(colorder as numeric)");
			...
			StringBuffer stringbuffer = new StringBuffer("insert into thry(");
			...
			for ( int i = 0 ; i<al_col.size() ; i++){
				...
				stringbuffer.append("'");
				stringbuffer.append(thry.getProp(Tools.nvl(al_col.get(i).toString())));
				stringbuffer.append("'");
				bl=true;
			}
			stringbuffer.append(")");
			...
			hp_op.put("event","增加");
			hp_op.put("tabname","thry");
			hp_op.put("cent","insert thry");
	...
	public Query findThryList(Query query,HttpServletRequest request)
		throws BussinessProcessException
	{
		DBTrans dbtrans = new DBTrans();
		Object obj = null;
		...
		try
		{
			CachedRowSet cachedrowset = dbtrans.getResultBySelect("select colname from tabmap where tabname = 'thry' order by CAST(colorder as numeric)");
			if (cachedrowset==null||cachedrowset.size()==0){
				log.info("*** === there is no relate information in tabmap!  === ***");
				throw new BussinessProcessException("no information in tabmap");
			}
			...
			{
				StringBuffer sbr_sql = new StringBuffer("select * from thry where 1=1 ");
			...
			stringbuffer1.append("select top ").append(query.getRowsPerPage()).append(" * from thry where id not in ");
			stringbuffer1.append("(select top ").append(i).append(" id from thry where 1=1 ").append(sbr_condition.toString()).append(" order by id desc ) ");
	...
	public Thry findThryById(String id,HttpServletRequest request)
		throws BussinessProcessException
	{
		DBTransUtil dbtransutil = new DBTransUtil();
		Object obj = null;
		Thry thry = new Thry();
		StringBuffer stringbuffer = new StringBuffer("select * from thry where id =");
		...
		try
		{
			...
			{
				thry.setid(cachedrowset.getString("id"));
				for (int j = 1; j <= i; j++)
					thry.setProp(resultsetmetadata.getColumnName(j).toLowerCase(), Tools.nvl(cachedrowset.getString(j)));
			}
		}
		catch (Exception exception)
		{
	...
	public String modifyThry(Thry thry,HttpServletRequest request)
		throws BussinessProcessException
	{
		DBTrans dbtrans = new DBTrans();
		StringBuffer stringbuffer = new StringBuffer("update thry set ");
		ArrayList al_col = new ArrayList();
		try
		{
		...
		////以下根据tabmap开启外键！
			...
			String iuik = tabmapdao.setcascade(dbtrans,"thry");
		////以上根据tabmap开启外键！
			CachedRowSet cachedrowset = dbtrans.getResultBySelect("select colname from tabmap where tabname = 'thry' and isautoinc='0' order by CAST(colorder as numeric)");
			if (cachedrowset==null||cachedrowset.size()==0){
				log.info("*** === there is no relate information in tabmap!  === ***");
			...
			String s_col = null;
			for (int i=0 ; i<al_col.size() ; i++){
				s_col = Tools.nvl((String)thry.getProp(al_col.get(i).toString()));
				if (s_col != null && !s_col.equals("")){
					if(i!=0){
						stringbuffer.append(",");
					}
			...
			stringbuffer.append(thry.getid());
			log.info(stringbuffer.toString());
			dbtrans.addSql(stringbuffer.toString());
			boolean flag = dbtrans.executeSql();
			
		////以下根据tabmap关闭外键！
			tabmapdao.remcascade(dbtrans,"thry",iuik);
		////以上根据tabmap关闭外键！
			...
			hp_op.put("event","修改");
			hp_op.put("tabname","thry");
			hp_op.put("cent","update thry id ="+thry.getid());
			syslogdao.logact(hp_op,request,dbtrans);
	...
	public String deleteThry(String id,HttpServletRequest request)
		throws BussinessProcessException
	{
		DBTrans dbtrans = new DBTrans();
		int i = Integer.parseInt(id);
		StringBuffer stringbuffer = new StringBuffer("delete from thry where id = ");
		...
		try
		{
			...
			hp_op.put("event","删除");
			hp_op.put("tabname","thry");
			hp_op.put("cent","delete thry id ="+id);
			syslogdao.logact(hp_op,request,dbtrans);
	...
	{
		DBTrans dbtrans = new DBTrans();
		StringBuffer stringbuffer = new StringBuffer("select * from tabmap where 1=1 and tabname='thry' order by CAST(colorder as numeric)");
		try
		{				
		...
		throws BussinessProcessException
	{
		...
		try{
			...
			if(hp_user != null ){
				...
				DBTrans dbtrans = new DBTrans();
				String sql ="select * from pritab where tabname='thry' and role_name='"+role_name+"'";
