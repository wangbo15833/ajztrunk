...
%>
<%
String [] col_name = {"id","tp_testnum","tp_carnum","tp_carname","tp_cartype","CMonitorStatus","Flag_bgzt","tp_cargdzt","tp_carproduction","tp_carouttime","tp_carcheckdate","tp_carcheckmeb","tp_carcheckmebphone","tp_testtoolnum","tp_teststarttime","TCjk_initTime","TCjk_startTime","TCjk_endTime","tp_testendtime","tp_testtotaltime","Tharvesttime","tp_testmeb","tp_testmebphone"};
String [] col_headname 	= {"id","测试编号","车辆编号","车辆名称","车辆型号","监控状态","检测报告状态","归档状态","车辆单位","出厂日期","抽检日期","生产厂试验负责人","生产厂试验负责人联系电话","测试仪器编号","测试起始时间","监控初始时间","监控开始时间","监控结束时间","测试结束时间","测试总时长","测试收割时长","鉴定站测试负责人","鉴定站负责人联系电话"};
%>
<head>
<title>试验车辆列表</title>
...
function confirmButton(operationMode, tr_row,id_value){
	...
	submitframe.location = "MainServlet_sys?fid=FCL00.00.00.05&id="+id_value;
	return ok;
}
</script>  
</head>
...
<tr id='sub_title' ><td align="center"><font size="+1">监控车辆列表</font></td></tr>
</table>
...
<tr   style="font-size:13px" ><!--  显示中文标题  -->
	<td width="20%" align="center" class="left_title_3" style="display:<%=operStyle%>" nowrap="nowrap">操作</td>
	<td width="20%" align="center" class="left_title_3" style="display:<%=operStyle_cancal%>" nowrap="nowrap">操作</td>
	<%
	for(int jk=0; jk<col_name.length; jk++){
	%>
	<td width="10%" align="center" nowrap="nowrap" style="padding-left:2px; padding-right:2px;" class="left_title_3"><span class="left_title_4" ><%=col_headname[jk]%></span></td>
	<%
	}
	%>
</tr>
<tr   style="font-size:13px" ><!--  显示英文标题  -->
	<td width="20%" align="center" class="left_title_3" style="display:<%=operStyle%>"></td>
	<td width="20%" align="center" class="left_title_3" style="display:<%=operStyle_cancal%>"></td>
	<%
	for(int jk=0; jk<col_name.length; jk++){
	%>
	<td width="10%" align="center" nowrap="nowrap" style="padding-left:2px; padding-right:2px;" class="left_title_3"><span class="left_title_4" ><%=col_name[jk]%></span></td>
	<%
	}
	%>
</tr>
...
%>
	...
	else
	{
		...
		<a href="MainServlet_sys?fid=FCL00.00.00.03&id=<%=crs.getString("id")%>" target="_self"><font color="#ff0000" class="tdrecord" style="font-size:12px" >修改</font></a>
<a onclick="return confirmButton('delete', <%=i_row%>, <%=crs.getString("id")%>);" target="submitframe"><font color="#ff0000" class="tdrecord" style="font-size:12px" >删除</font></a>
		...
		for(int jk=0; jk<col_name.length; jk++){
			...
			else if(col_name[jk].equals("Flag_bgzt")){	
				if(col_value.equals("0")){ 
			...
			else if(col_name[jk].equals("tp_cargdzt")){	
				if(col_value.equals("0")){ 
					col_value = "<div style='color:red;'>未归档</div>";
				}
				else if	(col_value.equals("1")){	
				    col_value = "<div style='color:blue;'>已归档</div>";
				}	
			}
			else if(col_name[jk].equals("tp_carouttime") || col_name[jk].equals("tp_carcheckdate") || col_name[jk].equals("tp_teststarttime")|| col_name[jk].equals("dataCreateTime")){
				col_value = Tools.formatDateToString_hms(col_value);
			}
			else if(col_name[jk].equals("TCjk_initTime") ){
				col_value = Tools.formatDateToString_hms(col_value);
			}
			else if(col_name[jk].equals("TCjk_startTime") ){
				col_value = Tools.formatDateToString_hms(col_value);
			}
			else if(col_name[jk].equals("TCjk_endTime") ){
				col_value = Tools.formatDateToString_hms(col_value);
			}
			else if(col_name[jk].equals("tp_testendtime") ){
				col_value = Tools.formatDateToString_hms(col_value);
			}
			%>
		<td align="center"   style="font-size:12px; padding-left:3px;padding-right:3px;" nowrap="nowrap" id="tr_<%=i_row%>_<%= col_name[jk]%>"><%= col_value%></td>
		 <%
		}
		%>		
	</tr>
