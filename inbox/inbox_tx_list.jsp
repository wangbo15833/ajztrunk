		...
		<td bgcolor='#dedfd7'>
			...
			<form name="frmList" target="submitframe" ></form>
				...
				else
				{
					...
					//---------------------------
				%>
				<tr bgcolor='FFFFFF' class='<%if(crs.getString("isread").equals("1")){%>font_px12_menu<%}else{%>font_px12_menu1<%}%>' id="tr_<%=Tools.nvl(crs.getString("id"))%>" onMouseOver="javascript:this.style.background= '#ECF2FC';" onMouseOut="javascript:this.style.background= '#FFFFFF';">
					<%
					for(int jk=0; jk<al_colname.size(); jk++){
						%>
						<td height="2" align="center" class="text3" nowrap="nowrap" id="tr_<%=Tools.nvl(crs.getString("id"))%>_<%=(String)(al_colname.get(jk))%>">&nbsp;&nbsp;<%=Tools.omit(crs.getString((String)al_colname.get(jk)),(String)al_colomit.get(jk))%></td>
						<%
					}
					%>
					<%
					...
					<td height="2" align="center" class="text3" nowrap="nowrap" id="tr_<%=i_row%>_edit2">
						...
						<%if(Integer.parseInt(mypri)>2){%><a href="#" onclick="modify('MainServlet_sys?fid=UII01.01.02.04&msgtype=<%=msgtype%>&id=<%=crs.getString("id")%>&mode=before')"><font color="#053080">查看</font></a>
						<%if(crs.getString("isread").equals("1")){%>
...
<div class="contextMenu" id="myMenu1" style="">
  <ul>
	...
	<!--li id="edit"><img src="./image2/icons/highlighter.png" /> edit</li-->
...
%>	if (document.getElementById('<%=key%>')!=null){		
		...
		if(window.addEventListener){ // Mozilla, Netscape, Firefox
			...
			document.getElementById('<%=key%>').addEventListener('onmouseout', alert('11'), false);
			//button.addEventListener('click', alert('12'), false);//执行顺序11 -> 12
		} else { // IE
			document.getElementById('<%=key%>').attachEvent('onmouseout', function(){alert('21');});
			//button.attachEvent('onclick', function(){alert('22');});执行顺序22 -> 21
		}
		
		*/
		
		//document.getElementById('<%=key%>').onMouseOut=function(){this.style.background= '#3399ff';};
		document.getElementById('<%=key%>').onmouseover=function(){this.style.background= '#ffaaaa';};
	}
	<%}}%>
	<%for (String key : mp_td.keySet()){
%>	if (document.getElementById('<%=key%>')!=null){
		if(document.getElementById('<%=key%>').innerHTML.indexOf('0')>0){
			document.getElementById('<%=key%>').innerHTML= '未读';
		}else if(document.getElementById('<%=key%>').innerHTML.indexOf('1')>0){
			document.getElementById('<%=key%>').innerHTML= '已读';
		}
	}
	<%}%>
}
...
$('.font_px12_menu').contextMenu('myMenu1', {
  bindings: {
	'see': function(t) {
	  ...
	  modify('MainServlet_sys?fid=UII01.01.02.04&msgtype=<%=msgtype%>&id='+t.id.split("_")[1]+'&mode=before')
	},
	'edit': function(t) {
	  alert('Trigger was '+t.id+'\nAction was edit');
	},
...
<script>
$('.font_px12_menu1').contextMenu('myMenu2', {
  bindings: {
	'see': function(t) {
	  //alert('Trigger was '+t.id+'\nAction was see');
	  modify('MainServlet_sys?fid=UII01.01.02.04&msgtype=<%=msgtype%>&id='+t.id.split("_")[1]+'&mode=before')
