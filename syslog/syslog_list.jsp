...
if(listMode.equals("")){
  ...
  System.out.println("11111111111111111111111111111111"+queryCon.getQueryString());
...
function confirmButton(operationMode, tr_row,id_value){
	...
	submitframe.location = "MainServlet_sys?fid=UII00.10.01.02&id="+id_value;
	//location=location;
}
</script> 
</head>
...
<table width='100%' border='0' align='center' cellpadding='1' cellspacing='1' bordercolor='#bdd7d6' bgcolor='#bdd7d6'  rules='none' style="font-size:12px;">
	<tr id='sub_title'><td align="center"><font size="+1">系统日志列表</font></td></tr>
</table>
	...
	<tr>
		<td bgcolor='#dedfd7'>
			...
			<form name="frmList" target="submitframe"></form>
				...
				%>
				<tr align="center"> 
					<td height="15" colspan="<%=al_colname.size()+1%>"><p align="center" class="text2">未查询到系统日志</p></td>
				</tr>
				<%
				}
				else
				...
				<tr bgcolor='FFFFFF' class='font_px12' id="tr_<%=i_row%>" onMouseOver="javascript:this.style.background= '#ECF2FC';" onMouseOut="javascript:this.style.background= '#FFFFFF';">
					...
					<%if(usertype.equals("系统管理员")){
				%>
				   <td align="center" width="18%" class="text3" nowrap="nowrap" ><a style="color:red;" onclick="confirmButton('delete',0,<%=crs.getString("id")%>)">删除此条日志</a></td>
				   <%
				   }else{%>
					<td height="2" align="center" class="text3" nowrap="nowrap" id="tr_<%=i_row%>_edit2"></td>
					<%}%>
				</tr>
					<%
						i_row++;
