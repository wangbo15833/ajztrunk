...
%>
<%
String [] col_name 	= {"id","tp_carproduction","tp_testnum","tp_carname","tp_carnum","tp_cartype", "tp_testtoolnum","tp_testtooltypenum","tp_teststarttime","tp_testendtime","tp_testtotaltime","tp_harvesthour","tp_testmeb","tp_testmebphone","TCjk_startTime","TCjk_endTime", "Nqw", "Nyb", "Nzd", "Nfqw", "Ntotal", "TAfirst","TAgzjg", "TArep", "TAef", "FAfirst", "FAgzjg", "FArep", "Faef", "result", "reporttime", "tp_reportoperator", "testcompany"};
String [] col_headname 	= {"id","车辆单位","测试编号","车辆姓名","车辆编号","车辆型号", "测试仪器编号","测试仪器型号","测试开始时间","测试结束时间","测试总时长","车辆收割时长","测试负责人","测试负责人电话","监控开始时间","监控结束时间", "轻微故障次数", "一般故障次数", "重大故障次数", "非轻微故障次数", "全部故障次数", "T首次故障前平均故障时间(分)"," T平均故障间隔时间(分)", "T平均修复时间(分)", "T有效度(%)", "F首次故障前平均故障时间(分)", "F平均故障间隔时间(分)", "F平均修复时间(分)", "F有效度(%)", "可靠性试验结论", "报告生成时间", "负责人", "检测单位"};
%>
<head>
<title>检测报告列表</title>
...
function confirmButton(operationMode, tr_row,id_value){
	...
	submitframe.location = "MainServlet_sys?fid=FCL00.00.04.05&id="+id_value;
	return ok;
}
</script> 
</head>
...
<tr id='sub_title'>
  <td align="center"><font size="+1">检测报告列表</font></td>
</tr>
</table>
...
<tr   style="font-size:13px" >
    <td width="20%" align="center" class="left_title_1" style="display:block;"></td>
...
%>	
	...
	else
	{
     ...
     <tr  bgcolor='FFFFFF' class='font_px12' id="tr_<%=i_row%>">
		<td align="center"  nowrap="nowrap" id="tr_<%=i_row%>_edit" style="display:block;">
		<!--<a href="jcbggl/report.html" target="_self"><font color="#ff0000" class="tdrecord" style="font-size:12px" >查看</font></a>-->
		<a href="MainServlet_sys?fid=FCL00.00.04.03&id=<%=crs.getString("id")%>" target="_self"><font color="#ff0000" class="tdrecord" style="font-size:12px">查看</font></a>
		</td>
		...
		<!--<a href="jcbggl/report.html" target="_self"><font color="#ff0000" class="tdrecord" style="font-size:12px" >查看</font></a>
		<a href="MainServlet_sys?fid=FCL00.00.04.03&id=<%=crs.getString("id")%>" target="_self"><font color="#ff0000" class="tdrecord" style="font-size:12px">查看</font></a>-->
		<a href="MainServlet_sys?fid=FCL00.00.04.03&id=<%=crs.getString("id")%>" target="_self"><font color="#ff0000" class="tdrecord" style="font-size:12px" >修改</font></a>
		<a href="MainServlet_sys?fid=FCL00.00.04.05&id=<%=crs.getString("id")%>" target="submitframe" onclick="return confirmButton('delete', <%=i_row%>, <%=crs.getString("id")%>);"><font color="#ff0000" class="tdrecord" style="font-size:12px" >删除</font></a>
		</td>
		<td align="center"  nowrap="nowrap" id="tr_<%=i_row%>_edit" style="display:<%=operStyle_cancal%>">
		<a href="MainServlet_sys?fid=FCL00.00.00.13&id=<%=crs.getString("id")%>" target="_self"><font color="#ff0000" class="tdrecord" style="font-size:12px" >重新生成</font></a>
		</td>
		<%
		...
		for(int jk=0; jk<col_name.length; jk++){
			...
			else if(col_name[jk].equals("reporttime") ){
				col_value = Tools.formatDateToString_hms(col_value);
			}
			%>
		...
		for(int ik=i_row; ik<=15; ik++){
			%>
     <tr  bgcolor='FFFFFF' class='font_px12'>
		 	<td align="center"  nowrap="nowrap" style="display:block;"></td>
